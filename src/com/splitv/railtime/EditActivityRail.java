...
import com.splitv.railtime.ViewShift.TimePickerFragment;
...
public class EditActivityRail extends SherlockFragmentActivity {
	...
	public static class TimePickerFragment extends SherlockDialogFragment
		...
		public Dialog onCreateDialog(Bundle savedInstanceState) {
			...
			String time = ((EditActivityRail)getActivity()).getFieldText();
			...
			String title = "";
			if(((EditActivityRail)getActivity()).curOpt=="startActivity"){
		...
		public void onTimeSet(TimePicker view, int hour, int minute) {
			...
			((EditActivityRail)getActivity()).updateTime(hour,minute);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mView=LayoutInflater.inflate(this, R.layout.activity_add_activity_rail);
		...
		EditText field = (EditText) mView.findViewById(R.id.add_rail_number);
		field.setText(dbResult.getString(dbResult.getColumnIndex(railDBHelper.activityTrack)));
		
		field = (EditText) mView.findViewById(R.id.add_rail_units);
		field.setText(dbResult.getString(dbResult.getColumnIndex(railDBHelper.activityUnits)));

		field = (EditText) mView.findViewById(R.id.add_rail_loaders);
		field.setText(dbResult.getString(dbResult.getColumnIndex(railDBHelper.activityLoaders)));
	...
	public boolean onCreateOptionsMenu(Menu menu) {
                ...
                builder.setPositiveButton(R.string.yes,
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {                            	
                            	//Setup DB helper
                        		helper = new railDBHelper( EditActivityRail.this);
                        		//get db instance
                        		...
                        		try {
                        			db.execSQL(query);
                        			//notify on success
                        			Toast.makeText( EditActivityRail.this, R.string.delete_shift_success, Toast.LENGTH_SHORT).show();
                        			dialog.dismiss();
                        			 EditActivityRail.this.setResult(RESULT_OK, null);
                            		finish();
                        		} catch (SQLException e) {
                        			e.printStackTrace();
                        			//notify on failure
                        			Toast.makeText( EditActivityRail.this, R.string.delete_shift_failed, Toast.LENGTH_SHORT).show();
                        			dialog.dismiss();
                        		}
                            }
                        });
	    ...
	    miPrefs.setOnMenuItemClickListener(new OnMenuItemClickListener() {	    	 
            ...
            public boolean onMenuItemClick(MenuItem item) { 
        		...
        		field =(EditText)mView.findViewById(R.id.add_rail_number);
        		String railNumber = (String) field.getText().toString();

        		field =(EditText)mView.findViewById(R.id.add_rail_units);
        		String railUnits = (String) field.getText().toString();
            	if(editTextEmpty(field)){
            		Toast.makeText(EditActivityRail.this, R.string.add_rail_empty_fields, Toast.LENGTH_SHORT).show();
        			return true;
            	}        		

        		field =(EditText)mView.findViewById(R.id.add_rail_loaders);
        		String railLoaders = (String) field.getText().toString();
            	if(editTextEmpty(field)){
            		Toast.makeText(EditActivityRail.this, R.string.add_rail_empty_fields, Toast.LENGTH_SHORT).show();
        			return true;
            	}
        		...
        		SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(EditActivityRail.this);

        		Float aHours = (float) 0;
        		if(prefs.getBoolean("pref_rail_enabled", true)){
            		
            		Float upm = Float.valueOf(prefs.getString("pref_rail_upm", "8"));
            		
            		aHours = ( Float.valueOf(railUnits) / upm ) / Float.valueOf( railLoaders);
        			
        		}
        		String activityHours = String.valueOf((rHours > aHours)?rHours:aHours);
        		
        		//Setup DB helper
        		railDBHelper helper = new railDBHelper(EditActivityRail.this);
        		//get db instance
        		...
        		args.put(railDBHelper.activityTrack, railNumber);	
        		args.put(railDBHelper.activityUnits, railUnits);		
        		args.put(railDBHelper.activityLoaders, railLoaders);		
    			...
    			if(db.update(railDBHelper.activityTable, args, strFilter, null)>0){
    				Toast.makeText(EditActivityRail.this, R.string.saved_rail_data, Toast.LENGTH_SHORT).show();
            			
        		}
        		else{
        			Toast.makeText(EditActivityRail.this, R.string.saved_rail_data_failed, Toast.LENGTH_SHORT).show();
